  ...
  <div
    ...
    <div class="w-full">
      ...
      <div class="grid gap-2">
        <div v-if="order.lineItems" v-for="item in order.lineItems.nodes" :key="item.product?.databaseId!"
          class="flex items-center justify-between gap-8">
          <img v-if="item.product?.node" class="w-16 h-16 rounded-xl"
            ...
            :title="item.variation?.node?.image?.title || item.product.node?.image?.title || 'Product image'" width="64"
